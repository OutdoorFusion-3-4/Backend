...
def process_csv_folder(folder_name, mapping_file):
    ...
    if csv_count < 2:
        process_all_files_in_folder(folder_path, process_single_csv, mapping, globalinstances)
    else:
        process_all_files_in_folder(folder_path, first_process_csv, mapping, globalinstances)
        process_all_files_in_folder(folder_path, second_process_csv, mapping, globalinstances)
        process_all_files_in_folder(folder_path, third_process_csv, mapping, globalinstances)
def process_all_files_in_folder(folder_path, processing_function, mapping, globalinstances):
    for filename in os.listdir(folder_path):
        process_csv_file(filename, folder_path, processing_function, mapping, globalinstances)
def process_csv_file(filename, folder_path, processing_function, mapping, globalinstances):
    ...
    with open(filename, 'r', encoding=get_file_encoding(filename)) as file:
        ...
        for row in reader:
            ...
            if not condition_met:
                return
def second_process_csv(filename, mapping, globalinstances):
    with open(filename, 'r', encoding=get_file_encoding(filename)) as file:
        reader = csv.DictReader(file)
        for row in reader:
            condition_met = False
...
def third_process_csv(filename, mapping, globalinstances):
